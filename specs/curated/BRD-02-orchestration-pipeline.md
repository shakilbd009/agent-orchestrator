...
-gest **FR-02-007: Strict parent completion.** A parent task MUST NOT reach `done` while any required child is unresolved, cancelled without approved scope reduction, or blocked. Parent completion MUST also require all blocking task-level gates to be approved.
...
|----|-----------|-------------|
| AC-02-001 | With `platform-orchestration=false`, platform-native project/task/gate mutation endpoints are disabled | Feature flag integration test |
