...
|---|---|---|---|---|---|---|
| `orchestration-ui` | UI | product (orchestration) | Internal orchestration dashboard with the **live magnetic graph (`/orchestration/live`) as the primary surface**: visualizes real agent activity as emergent spring/focal clusters driven by the SSE stream, with **per-task centroids** (concurrent active tasks form separate temporary clusters), a **node/cluster click → gate/approval drawer** that reuses the existing phase/task gate REST endpoints, and a user-toggleable **static knowledge-map mode**. Board, gates, decomposition, and webhooks remain as secondary tabs. Browser bundle. | `frontend/src/routes/orchestration/**`, `frontend/src/routes/+layout.svelte`, `frontend/src/routes/+page.svelte`, `frontend/src/lib/api/**` (shared TS API/SSE client — also used by `client-portal-ui`), `frontend/src/lib/orchestration/**`, `frontend/src/lib/magnetic/**` (living-graph engine + SSE event-feed reducer + deterministic agent homes) | Consumes BRD-02 REST + SSE; reads `VITE_FF_ENABLE_PLATFORM_ORCHESTRATION`. Trust boundary: browser → backend over `X-Actor-ID` / `X-Actor-Role` headers. The magnetic graph reuses the shared SSE client (no new transport/contract) and the DEV harness's `agent.*`/`handoff.*` topics; the click→gate drawer calls the same phase/task gate endpoints as `/orchestration/gates` (no gate logic reimplemented). | `orchestration-api`, `frontend-shell` |
...
- **Magnetic graph is a faithful engine port.** `orchestration-ui` ships a
  ...
  Phase E promoted it to the primary surface: the reducer emits **per-task
  clusters** so each active task gets its own focal centroid (concurrent tasks no
  longer collapse into one merged blob); node/cluster **click** opens a gate /
  approval drawer that calls the existing phase/task gate REST endpoints (gate
  logic is reused, not reimplemented); and the reduced-motion **static
  knowledge-map** is exposed as a first-class user toggle (not only
  `prefers-reduced-motion`). When no backend/SSE is reachable the page drives the
  same reducer from a scripted mock fixture (`startMockFeed`) so the wiring is
  demonstrable without Postgres+Redis. This is additive UI only (no approval
  boundary crossed).
